...
def main(argv=None) -> None:
    ...
    calls = select_calls(repos, args.calls)

    for call in calls:
        repos.evals.tag_eval_run(call["call_id"], args.label)
    print(f"Tagged {len(calls)} call(s) with eval_run_label={args.label!r} (--calls {args.calls})")

    deterministic = run_deterministic_pass(repos, calls)
    print("\nDeterministic stats:")
    print(json.dumps(deterministic, indent=2))

    batch_results = run_classification_pass(repos, calls, args.label)

    error_rates = compute_error_rates(repos, args.label)
    print("\nError rates:")
    for class_id, rate in sorted(error_rates.items()):
        print(f"  {class_id}: {rate:.1%}")

    suggestions = run_taxonomy_critique(repos, batch_results, args.label)
    print(
        f"\n{len(suggestions)} new taxonomy_suggestions row(s) for label={args.label!r} — "
