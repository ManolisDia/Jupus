...
def _print_report(results: list[dict]) -> None:
    ...
    print(
        f"\nVerdict: the system holds up cleanly through N={holds_through} "
        f"(per-call median latency stayed within {DEGRADATION_MULTIPLE}x of N={results[0]['n']}'s "
        f"{baseline:.1f}ms baseline)."
    )
    if any(r["cross_call_leakage_found"] for r in results):
        print("WARNING: cross-call state leakage was detected at one or more N levels — see per-N table above.")


async def run_all_levels(n_levels: tuple[int, ...], mode: str, repos: Repositories) -> list[dict]:
    results = []
    for n in n_levels:
        CALL_STATES.clear()
...
def main(argv=None) -> None:
    ...
    if os.path.exists(STRESS_DB_PATH):
        os.remove(STRESS_DB_PATH)
    reset_schema(connect(STRESS_DB_PATH))
    stress_settings = settings.model_copy(update={"db_path": STRESS_DB_PATH})
    repos = get_repositories(stress_settings)
