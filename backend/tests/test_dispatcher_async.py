...
async def test_greeting_start_chains_into_next_node_same_dispatch(repos):
    ...
    with (
        ...
        reply, _stage = await run_supervisor_turn(repos, "call-1", "tool-1", "I need help, my boss fired me")

    assert call_count["n"] == 2
    assert reply == "Got it — this falls under employment law."
...
async def test_greeting_chain_not_triggered_when_explicit_request_escalates(repos):
    ...
    with patch("backend.dispatcher.GRAPH.invoke", side_effect=fake_invoke):
        reply, _stage = await run_supervisor_turn(repos, "call-1", "tool-1", "let me talk to a person")
...
async def test_faq_tangent_answered_even_when_turn_otherwise_succeeds(repos):
    ...
    with (
        ...
        reply, _stage = await run_supervisor_turn(
            repos, "call-1", "tool-1",
            "My boss is trying to fire me. Also, are you open on weekends?",
        )

    assert "Monday to Friday" in reply
    assert "Got it — employment law." in reply
...
async def test_faq_not_triggered_for_unrelated_utterance(repos):
    ...
    with (
        ...
        reply, _stage = await run_supervisor_turn(repos, "call-1", "tool-1", "My boss is trying to fire me.")

    assert reply == "Got it — employment law."
...
async def test_concurrent_calls_for_same_call_id_serialize(repos):
    ...
    with patch("backend.dispatcher.GRAPH.invoke", side_effect=fake_invoke):
        t1 = asyncio.create_task(run_supervisor_turn(repos, "call-1", "tool-1", "first"))
        t2 = asyncio.create_task(run_supervisor_turn(repos, "call-1", "tool-2", "second"))
...
async def test_exception_during_lock_hold_still_releases_lock(repos):
    ...
    _seed_state("call-1", stage="ended")
    reply, _stage = await asyncio.wait_for(
        run_supervisor_turn(repos, "call-1", "tool-2", "hi again"), timeout=1.0
    )
    assert reply == "This call has already been completed."
...
async def test_disconnect_waits_for_in_flight_turn_before_marking_abandoned(repos):
    ...
    with patch("backend.dispatcher.GRAPH.invoke", side_effect=slow_invoke):
        turn_task = asyncio.create_task(run_supervisor_turn(repos, "call-1", "tool-1", "hi"))
