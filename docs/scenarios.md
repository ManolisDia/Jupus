...
Each scenario in `backend/tests/test_scenarios.py` should be its own `test_scenario_s1_...` through `test_scenario_s6_...` function (not one giant parametrized blob) so a failure names the exact scenario that broke, and so the manual Phase 7 checklist can reference these by the same S1–S6 names.
