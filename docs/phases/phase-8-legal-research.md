...
- [ ] `backend/supervisor/knowledge/{employment,tenancy,immigration}_statutes.json` authored, 8–12 entries each, all England & Wales, each with a real (hand-written-from-general-knowledge, not scraped) `citation`/`text`.
- [ ] `backend/supervisor/knowledge/search.py`'s BM25 implementation, `node_research_gather`, `node_research_deliver`, `looks_like_research_skip`, and the dispatcher's `_search_statutes_in_background`/`_reconcile_statute_search`/`STATUTE_SEARCHES` implemented per the design above.
- [ ] `pytest` (full suite) passes with zero failures, including every test listed above and both new `test_scenarios.py` S7 variants.
...
- [ ] `docs/DECISIONS.md` entry added: jurisdiction (England & Wales), corpus provenance (hand-authored from general knowledge for this take-home, not independently verified against primary sources), and the BM25-floor + closed-set-grounding design as the two things that keep this feature from ever inventing a citation.
