...
**Known gap, not part of this phase's DoD**: the admin live "supervisor mind" graph visualization (`admin/graph.js`, a Phase 7/8-era optional stretch in `docs/phases/phase-9-polish-submission.md`) still hardcodes the pre-Phase-8 node layout (`capture → booking` directly) and hasn't been updated to show the new `research` stage — it will render a stale/incomplete picture of a call that goes through research until that's done separately. Not required for this phase's DoD (it's an optional visualization layer, not the pipeline itself), but flagged here so it isn't mistaken for an oversight later.
