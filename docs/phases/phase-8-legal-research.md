...
- [x] `admin/graph.js`/`admin/graph.html` (the optional live "supervisor mind" visualization, `docs/phases/phase-13-polish-submission.md`) updated to show the new `research` stage — a "Research" box between "Capture (confirm)" and "Booking", `node_research_gather`/`node_research_deliver` both mapped onto it, new edges/loop, and `describeTransition` branches for the capture→research handoff and both research outcomes (citation found / not found / caller skipped). Not itself required by this phase's DoD, but was left stale after the initial build and fixed afterward rather than left silently misleading.
- [x] `heuristics.looks_like_bare_affirmation` added and wired into `node_research_gather` after a live session showed a real gap: because `_enter_research` asks the intro question in the same turn as the capture→research transition (deliberately, for latency), a caller's trailing reaction to the *previous* question (e.g. confirming their phone number) can arrive as the research node's next utterance with no gap to "catch up" on the new question — and `node_research_gather` had no plausibility gate at all, unlike `node_capture_fast`'s `looks_like_tangent`/shape checks. A bare affirmation ("yep, that's correct") now gets one re-ask instead of being silently treated as the substantive answer and burning the search on it. See `docs/fixes/2026-08-24-008.md`.
