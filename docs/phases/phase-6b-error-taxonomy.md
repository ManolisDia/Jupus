...
- [ ] `python backend/db/seed_demo_calls.py` runs clean, now produces 8 calls with the expected variety.
- [ ] `python eval/run_eval.py --label smoke_test` runs clean: prints deterministic stats + error rates, populates `call_error_flags`.
- [ ] `pytest eval/tests backend/tests/test_seed_demo_calls.py backend/tests/test_admin_routes.py` — all pass.
- [ ] Each of the 4 error-class-specific seeded calls is confirmed flagged with its intended class (spot-check in the DB — the mocked unit tests only prove the harness works, not that a real judge call classifies correctly).
- [ ] Manual: `/admin` shows error-class badges on the calls list; clicking a flagged call shows its evidence text.
