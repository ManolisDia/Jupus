...
- [x] `python eval/run_eval.py --label smoke_test` runs clean: prints deterministic stats + error rates, populates `call_error_flags`. (Verified live with a real `ANTHROPIC_API_KEY`: `python -m eval.run_eval --label live-verify-phase6` tagged 8 calls, printed sensible deterministic stats and non-trivial error rates, and populated real `call_error_flags` rows.)
- [x] `pytest eval/tests backend/tests/test_seed_demo_calls.py backend/tests/test_admin_routes.py` — all pass.
- [ ] Each of the 4 error-class-specific seeded calls is confirmed flagged with its intended class (spot-check in the DB — the mocked unit tests only prove the harness works, not that a real judge call classifies correctly). **Attempted for real, did not pass — not a credentials issue anymore.** The real judge flagged 0 of the 4 hand-seeded error-class calls, because `classify_call_errors` reads only `trace`/outcome/escalation_reason as evidence and `seed_demo_calls.py` never writes any `trace_events` for its hand-authored rows (by design — it bypasses the live pipeline). See `docs/known-issues/2026-08-24-001.md` for the full root cause and unresolved options. The real judge *does* work correctly against calls that have real trace_events — `eval/replay_scenarios.py`'s live run (see phase-6c DoD) is the meaningful proof of that, and is the recommended thing to point to instead of this seeded-data check.
