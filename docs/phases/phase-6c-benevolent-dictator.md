...
- [ ] `python eval/run_eval.py --label smoke_test` runs clean, now also populates `taxonomy_suggestions`. **BLOCKED — same reason as Phase 6b's identical item: `propose_taxonomy_updates` is a real Claude call and this environment has no real `ANTHROPIC_API_KEY`.** `run_taxonomy_critique`'s own logic (including that it writes `taxonomy_suggestions` rows) is fully unit-tested with the Claude call mocked.
- [x] `pytest eval/tests backend/tests/test_annotation_routes.py backend/tests/test_admin_routes.py` — all pass.
- [ ] Manual: `python eval/replay_scenarios.py --label baseline` against the real pipeline (live Claude/OpenAI calls) succeeds and produces 6 tagged calls. **Update after rebasing onto Phase 4/5:** the code-side blocker is gone — `node_booking`/`node_escalation` are real and merged, and the script was ported off the removed `on_ask_supervisor` name onto the real `dispatcher.process_supervisor_call` entry point (see `eval/replay_scenarios.py`'s module docstring and git history). **STILL BLOCKED, now purely on credentials** — this environment's `.env` only has placeholder `OPENAI_API_KEY`/`ANTHROPIC_API_KEY` values (placeholder strings, not real ones), not real ones, so a real run was not attempted; running it against placeholder keys would just produce upstream API failures, not a meaningful check. The harness itself (creates + tags exactly 6 calls, one per scenario_id) is verified in `eval/tests/test_replay_scenarios.py` with the dispatcher fully mocked, per the phase doc's own "harness only" instruction for that test file.
