...
- [ ] `python backend/db/seed_demo_calls.py` runs clean, now includes the 2 pre-populated annotations.
- [ ] `python eval/run_eval.py --label smoke_test` runs clean, now also populates `taxonomy_suggestions`.
- [ ] `pytest eval/tests backend/tests/test_annotation_routes.py backend/tests/test_admin_routes.py` — all pass.
- [ ] Manual: `python eval/replay_scenarios.py --label baseline` against the real pipeline (live Claude/OpenAI calls) succeeds and produces 6 tagged calls.
- [ ] Manual: `python eval/compare_runs.py --baseline baseline --candidate baseline` (comparing a label against itself) reports zero regressions and exit code 0.
- [ ] Manual: open `/admin/annotate`, confirm seeded unreviewed calls appear, submit a review (a couple of classes checked, one "doesn't fit any class" note, marked gold), confirm it disappears from the queue and shows correctly in `/api/calls/{id}`.
- [ ] Manual: `python eval/calibrate_judge.py` against the seeded pre-annotated calls prints per-class precision/recall (or raw counts) matching what you'd compute by hand, including the deliberate disagreement case.
- [ ] Manual: `/admin`'s taxonomy-suggestions panel renders with approve/reject buttons; approving one updates its status without touching `eval/error_classes.py` automatically.
