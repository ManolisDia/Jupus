...
- [ ] Manual: `python eval/replay_scenarios.py --label baseline` against the real pipeline (live Claude/OpenAI calls) succeeds and produces 6 tagged calls. **BLOCKED — needs real OpenAI/Anthropic keys (this environment has neither) AND Phase 4/5's real booking/escalation nodes for S2/S3/S5/S6 to behave as documented (see the script's own module docstring).** The harness itself (creates + tags exactly 6 calls, one per scenario_id) is verified in `eval/tests/test_replay_scenarios.py` with the dispatcher fully mocked, per the phase doc's own "harness only" instruction for that test file.
