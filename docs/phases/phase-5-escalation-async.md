...
- [x] Manual live call: describe a genuinely multi-area issue (e.g. an employment dispute tangled with a visa status question) — confirm immediate escalation with `out_of_scope_multi_area`, not a retry loop. Confirmed — `call_id=3f8c7364-d096-4153-aac5-e024e00b09c2`: `classify_practice_area` → `{'area': 'multiple_areas', confidence: 0.85}` on the first call, `retry_counts` untouched, immediate escalation.
