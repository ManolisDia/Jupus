...
- [x] `pytest backend/tests/test_heuristics.py backend/tests/test_escalation_node.py backend/tests/test_routing_node.py backend/tests/test_dispatcher_async.py` — all pass. Verified in a worktree built off `master` (equivalent to `phase-4-booking` at the time, since Phase 4 had no commits yet) while Phase 4 was still in progress elsewhere — see note below.
- [ ] Manual live call: say "can I just talk to a real person" as your very first utterance — confirm immediate escalation (no routing/capture questions asked first), and `docs/handoffs/<call_id>.md` is written. **Blocked**: needs a live mic/browser session against a running backend — not done from this worktree.
- [ ] Manual live call: describe a genuinely multi-area issue (e.g. an employment dispute tangled with a visa status question) — confirm immediate escalation with `out_of_scope_multi_area`, not a retry loop. **Blocked**: same as above.
- [ ] **Manual live call — the core async requirement:** trigger a supervisor call (e.g. ask to book something), and immediately speak a follow-up question before the first reply comes back. Confirm: (a) the follow-up is heard and responded to without waiting for the first call to resolve, and (b) the first call's result is spoken only once there's a natural gap, not talked over the follow-up. Record this specific interaction — it's the one to show in the video. **Blocked**: needs both a live session and the real (non-stub) `node_booking` from Phase 4 to make "ask to book something" meaningful; the async defer/deliver mechanics themselves are covered by `backend/tests/test_dispatcher_async.py` and `test_bridge_relays_speech_started_and_stopped` in the meantime.
- [ ] Manual: check the backend log during the above test — confirm you can see the first task's result being deferred (`SPEAKING=True`) and then delivered on the next `speech_stopped` event, not silently dropped or double-spoken. **Blocked**: depends on the live test above.
- [ ] Manual: temporarily make a deterministic tool raise on purpose (e.g. a one-line edit to `SlotRepository.book` to always raise `RuntimeError`), attempt a live booking, confirm you hear the graceful fallback and a clean escalation rather than dead air or a hung call — then revert the temporary edit. This is specifically testing the catch-all, not the `LLMCallFailed` path already covered elsewhere. **Blocked**: `node_booking`/`SlotRepository.book` don't exist yet in this worktree (Phase 4 not merged); the equivalent catch-all path (unhandled exception from `GRAPH.invoke`) is covered automatically by `test_unexpected_exception_in_graph_invoke_delivers_fallback_not_dead_air` etc. in `test_dispatcher_async.py` — re-run this manual check once Phase 4 lands.
- [x] `docs/fixes/` or `docs/known-issues/` has an entry if anything about the async timing behaved unexpectedly during manual testing — don't let a flaky-but-passing manual check go unrecorded. See `docs/fixes/2026-08-21-004.md` — a real deferred-reply staleness bug was caught and fixed while writing the automated bridge test.

**Note on how this phase was built**: started in `.claude/worktrees/phase-5-escalation-async` (branched from `master`, which had no Phase 4 commits yet — Phase 4 was still in-progress/uncommitted in the main checkout at the time) at the user's explicit request, to get ahead on Phase 5's independent pieces while Phase 4 finished elsewhere. `node_booking` is therefore still Phase 3's stub here. Everything not gated on a real booking flow or a live mic session is implemented and tested: `heuristics.py`, the `multiple_areas` routing branch, the real `node_escalation` + handoff-note writing, and the full async dispatcher rewrite (locks, `SPEAKING`/`DEFERRED`/`CONNECTIONS`, deferred delivery, staleness handling, the unhandled-exception catch-all, and cross-cutting.md section 2's disconnect cleanup). Before merging: rebase/replay this branch onto the finished `phase-4-booking`, wire `no_acceptable_slot` (set by the real `node_booking`) into whatever escalation handling doesn't already cover it for free, and run the still-blocked manual DoD items above.
