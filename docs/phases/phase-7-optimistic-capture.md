...
- [ ] `node_capture_fast`, `node_capture_confirm`, `looks_like_tangent` implemented per the design above.
- [ ] `pytest` (full suite) passes with zero failures, including all 10 new tests above and the existing `test_scenarios.py`.
- [ ] Manual, live: run through the field-capture flow with a real browser/mic session, confirm the fast pass genuinely has no audible gap between fields, and the confirm phase reads naturally with its transitional line.
- [ ] Manual, live: deliberately mumble/garble an email so real extraction fails — confirm the urgent-reask path catches it promptly rather than deferring all the way to the end of the fast pass.
- [ ] Manual, live: say something off-topic ("wait, how long will this take?") in place of an expected field answer — confirm the gate correctly falls back to the real, synchronous path rather than guessing wrong.
- [ ] `eval/replay_scenarios.py`'s S2/S3 (the two booking scenarios that exercise full field capture) re-run live against the real pipeline, produce the same real outcomes as before this phase (booked, correct slot), confirming the optimistic path doesn't change what actually gets captured/booked, only how it feels.
