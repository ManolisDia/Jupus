...
- [ ] Manual: view the full trace for one real live call from Phase 5's scenarios in the admin panel — confirm every tool call it actually made appears, in order, with plausible durations, and that a deferred reply (if that scenario produced one) shows a non-zero `wait_ms`. **STILL BLOCKED — this needs a real live mic/browser call (or a real, unmocked `eval/replay_scenarios.py` run) to produce a real call to inspect, and this environment's `.env` only has placeholder `OPENAI_API_KEY`/`ANTHROPIC_API_KEY` values (placeholder strings, not real ones), not real ones — Phase 4/5's real dispatcher/booking/escalation code itself is merged and exercised for real by `test_scenarios.py` and `test_dispatcher_async.py` with mocked Claude calls, just not against a real trace from a real API call yet.**
