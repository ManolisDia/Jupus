...
- [x] Manual (partial — see caveat): view the full trace for one real live call from Phase 5's scenarios in the admin panel — confirm every tool call it actually made appears, in order, with plausible durations, and that a deferred reply (if that scenario produced one) shows a non-zero `wait_ms`. (Verified live with real API keys: `GET /api/calls/replay-s2-fd85c99c/trace` — one of `eval/replay_scenarios.py`'s real S2 run — returns 66 correctly-ordered events by `seq`, with real `node_entered`/`tool_call_start`/`tool_call_end`/`node_exited` events, real tool names/args in the payload, and plausible durations (~1.5-3s for real Claude calls, ~10ms for deterministic ones).
  ...
  **Caveat, now with a real architecture fix behind it and one remaining blocker:** while live-testing the non-zero-`wait_ms` half of this check, found and fixed a real bug — `dispatcher.process_supervisor_call` called `GRAPH.invoke` (which makes real, synchronous, blocking Anthropic SDK calls with no internal `await`) directly on the event loop, which fully froze the entire server — including the `/bridge` WebSocket's own read loop for `speech_started`/`speech_stopped` VAD events — for the duration of every real tool call. That meant the deferred-delivery path could never actually trigger against real timing, no matter how a caller spoke, since the server was never listening for the VAD signal at the moment it mattered. Fixed by wrapping both `GRAPH.invoke` call sites in `asyncio.to_thread` (see git history) — verified the fix doesn't break anything (full suite green, `eval/replay_scenarios.py`'s real 6-scenario run still lands on correct outcomes).

  **Still can't get a live non-zero `wait_ms` through the actual browser client**, but for a separate, pre-existing, already-documented reason: `docs/known-issues/2026-08-22-001.md` (the `interrupt_response` response-collision race) fires reliably under the exact "talk again immediately, no gap" technique this check requires, tearing down the Realtime session before a trace can be captured. That's Phase 5 client-side territory with its own history (a prior fix attempt for it was tried and reverted after causing a worse regression), out of scope to reopen here. Decision: accept this as a known, separately-tracked limitation rather than block the Phase 6 merge on it — the deferred-delivery *mechanism* (`deliver_or_defer`/`drain_deferred`) is unit-tested directly, and is now also structurally capable of triggering for real (the event-loop-freezing bug that would have silently prevented it either way is fixed), even though proving it end-to-end live is blocked on 2026-08-22-001 instead.)
...
- [x] Manual: kill the browser tab mid-call, confirm the backend marks it abandoned and the registries (`CONNECTIONS`/`SPEAKING`/`DEFERRED`) are cleared. (Verified live by the user through the real browser client: started a call, reached `capture`, closed the tab mid-conversation on call_id `16e0423a-9108-413b-b160-3972acc2119e` — confirmed via `/admin` that its outcome shows `abandoned`. Registry-clearing itself isn't independently observable through the UI, so the indirect check — a fresh subsequent call behaving completely normally — plus `test_dispatcher_async.py::test_disconnect_clears_registries`'s direct assertion that `mark_call_abandoned` clears all three registries, together cover this.)
- [x] Manual: temporarily break `ANTHROPIC_API_KEY`, attempt a live call, confirm a graceful fallback reply rather than dead air, and `system_error` escalation after 3 consecutive failures. (Verified live by the user through the real browser client with the key deliberately broken: real trace captured showing the exact designed sequence — turn 1 fails (401 auth error, retried once, still fails) → graceful fallback "could you say that again?", stage stays `routing`; turn 2 same; turn 3 fails → escalates, stage `routing → escalation`, reply becomes "I'm having some technical trouble... let me get you to a person." All 3 consecutive-failure counting and both fallback/escalation replies matched `_llm_failure_fallback`'s logic exactly. Real key restored from backup afterward and confirmed back to its original length.)
