...
- [x] `pytest backend/tests/test_llm_utils.py backend/tests/test_system_error_escalation.py backend/tests/test_dispatcher_async.py backend/tests/test_scenarios.py` — all pass (cross-cutting closeout), **with two noted exceptions**: `test_dispatcher_async.py::test_disconnect_clears_registries` is skipped (CONNECTIONS/SPEAKING/DEFERRED are Phase 5's async-dispatcher registries, not merged into this branch), and `test_scenarios.py` only implements S1/S4 for real — S2/S3 need Phase 4's real booking node, S5/S6 need Phase 5 additions (`multiple_areas` classification value, `is_explicit_human_request` heuristic); those 4 are explicit skips with reasons, not faked passes.
...
- [ ] Manual: view the full trace for one real live call from Phase 5's scenarios in the admin panel — confirm every tool call it actually made appears, in order, with plausible durations, and that a deferred reply (if that scenario produced one) shows a non-zero `wait_ms`. **BLOCKED — Phase 5's scenarios/live dispatcher aren't merged into this branch; the trace endpoint itself is verified working (ordered by seq) against synthetic trace_events in `test_admin_routes.py`, but there is no real live call to inspect yet.**
...
- [ ] Manual: kill the browser tab mid-call, confirm the backend marks it abandoned and the registries (`CONNECTIONS`/`SPEAKING`/`DEFERRED`) are cleared. **BLOCKED — no live browser/mic session possible in this environment, and the registries themselves don't exist yet (Phase 5). The non-registry half (`mark_call_abandoned` sets `outcome="abandoned"` and doesn't override an already-ended call) is unit-tested in `test_dispatcher_async.py`.**
- [ ] Manual: temporarily break `ANTHROPIC_API_KEY`, attempt a live call, confirm a graceful fallback reply rather than dead air, and `system_error` escalation after 3 consecutive failures. **BLOCKED — requires a live browser/mic call against real OpenAI/Anthropic keys, not available in this environment. The underlying behavior (graceful fallback on 1-2 failures, `system_error` escalation on the 3rd) is unit-tested end-to-end in `test_system_error_escalation.py` and per-node test files with mocked failures.**
