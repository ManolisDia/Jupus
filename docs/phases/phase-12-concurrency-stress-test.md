...
- [x] Optional: one small, capped live-API run (`--mode live --n-levels 5` or similar), its real numbers quoted directly in the video alongside the mocked report, clearly labeled as the one live data point among otherwise-mocked evidence. Run 2026-08-25 after confirming billing spend caps were set: wall-clock 2937ms for 5 concurrent real Claude calls vs. ~2203ms for one call's own median, zero cross-call mixing in stored transcripts — see `docs/DECISIONS.md` and `docs/answers.md` Q3.
